...
namespace UltraSim.ECS.GUI
{
    ...
    public partial class ECSControlPanel : Control
    {
        ...
        private void BuildUI()
        {
            ...
            GD.Print($"╔══════════════════════════════════════════════╗");
            GD.Print($"║ ECSControlPanel BuildUI                      ║");
            GD.Print($"╠══════════════════════════════════════════════╣");
            GD.Print($"║ Viewport: {viewportSize.X}x{viewportSize.Y}");
            GD.Print($"║ ECSControlPanel Size: {Size}");
            GD.Print($"║ Padding: {padding}px");
            GD.Print($"╚══════════════════════════════════════════════╝");
            ...
            _showTimingsCheckBox.Text = "Show Timings";
        ...
        public void RefreshSystemsList()
        {
            ...
            foreach (var system in _systemManager.GetAllSystems())
            {
                ...
                if (settings != null)
                {
                    GD.Print($"  âœ“ {system.Name} has settings");
                    CreateSystemEntry(system);
                    systemsWithSettings++;
                }
                else
                {
                    GD.Print($"  âœ— {system.Name} has NO settings");
                }
            }
        ...
        private void CreateSystemEntry(BaseSystem system)
        {
            ...
            GD.Print($"  â†’ Added SystemEntryUI to tree, total children: {_systemsContainer.GetChildCount()}");
        }
        ...
        {
            GD.Print($"\n╔══════════════════════════════════════════════╗");
            GD.Print($"║ ForceLayoutUpdate                            ║");
            GD.Print($"╠══════════════════════════════════════════════╣");
            GD.Print($"║ ECSControlPanel Size: {Size}");

            if (_mainPanel != null)
            {
                GD.Print($"║ MainPanel Size: {_mainPanel.Size}");
                GD.Print($"║ MainPanel Position: {_mainPanel.Position}");
                ...
                GD.Print($"║ After Reset - MainPanel: {_mainPanel.Size}");
            }
            else
            {
                GD.Print($"║ ERROR: MainPanel is null!");
            }
        ...
        {
            GD.Print($"\n╔══════════════════════════════════════════════╗");
            GD.Print($"║ Final Layout (After Show)                    ║");
            GD.Print($"╠══════════════════════════════════════════════╣");
            GD.Print($"║ ECSControlPanel: {Size}");

            // Get the root margin
            var rootMargin = GetChildOrNull<MarginContainer>(0);
            if (rootMargin != null)
            {
                GD.Print($"║ RootMargin: {rootMargin.Size}");
            }

            if (_mainPanel != null)
            {
                GD.Print($"║ MainPanel: {_mainPanel.Size}");
                GD.Print($"║ MainPanel Position: {_mainPanel.Position}");
            }

            if (_scrollContainer != null)
            {
                GD.Print($"║ ScrollContainer: {_scrollContainer.Size}");
            }

            if (_systemsContainer != null)
            {
                GD.Print($"║ SystemsContainer: {_systemsContainer.Size}");
            }
    ...
    internal partial class SystemEntryUI : PanelContainer
    {
        ...
        private void OnApplyPressed()
        {
            ...
            _isDirty = false;
            _applyButton.Visible = false;
            GD.Print($"âœ“ Applied settings for {_system.Name}");
        }
