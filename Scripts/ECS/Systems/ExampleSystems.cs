...
namespace UltraSim.ECS.Examples
{
    ...
    /// <summary>
    /// Manual save system - saves all system state when invoked.
    /// Also provides auto-save configuration through settings.
    /// </summary>
    public class SaveSystem : BaseSystem
    {
        ...
        {
            public BoolSetting AutoSaveEnabled { get; private set; }
            public FloatSetting AutoSaveInterval { get; private set; }
            public BoolSetting SaveOnExit { get; private set; }

            public Settings()
            {
                AutoSaveEnabled = RegisterBool("Auto-Save Enabled", false,
                    tooltip: "Automatically save all systems at regular intervals");

                AutoSaveInterval = RegisterFloat("Auto-Save Interval", 300.0f, 10.0f, 3600.0f, 10.0f,
                    tooltip: "Time in seconds between auto-saves (10s - 1 hour)");

                SaveOnExit = RegisterBool("Save On Exit", true,
                    tooltip: "Automatically save when the game exits");
            }
        }
...
#if USE_DEBUG
            GD.Print($"[SaveSystem] Initialized. Auto-save: {SystemSettings.AutoSaveEnabled.Value}, " +
                     $"Interval: {SystemSettings.AutoSaveInterval.Value}s");
#endif
        }
        ...
        public override void Update(World world, double delta)
        {
            ...
            GD.Print("\n======================================");
            GD.Print($"  💾 SAVE SYSTEM - SAVE #{_saveCount}");
            GD.Print("======================================");

            var systemManager = world.Systems;
            var allSystems = systemManager.GetAllSystems();

            int systemsSaved = 0;
            int settingsSaved = 0;

            // Save all systems using their Serialize() method
            foreach (var system in allSystems)
            {
                // Call system's Serialize() method for game state
                system.Serialize();
                systemsSaved++;

                // Save settings if system has them
            ...
            // Save master ECS config
            systemManager.SaveMasterSettings();

            GD.Print($"✓ Saved {systemsSaved} system states");
            GD.Print($"✓ Saved {settingsSaved} system settings");
            GD.Print($"✓ Master ECS configuration saved");
            GD.Print("======================================\n");
        }

        /// <summary>
        /// SaveSystem doesn't have state to serialize.
        /// Its settings are already handled by SaveSettings().
        /// </summary>
        public override void Serialize()
        {
            // No state to save - settings handled separately
        }

        /// <summary>
        /// SaveSystem doesn't have state to deserialize.
        /// </summary>
        public override void Deserialize()
        {
            // No state to load - settings handled separately
        }
    }
