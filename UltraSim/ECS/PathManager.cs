...
namespace UltraSim.IO
{
    ...
    public static class PathManager
    {
        ...
        public static void Initialize(string? customBasePath = null)
        {
            ...
            _savePath = Path.Combine(_basePath, "Saves");
            _settingsPath = Path.Combine(_basePath, "Settings");
        ...
        public static string ResolvePath(string path)
        {
            ...
            // Handle special prefixes
            if (path.StartsWith("save://"))
                return Path.Combine(SavePath, path.Substring(7));
            
            if (path.StartsWith("settings://"))
                return Path.Combine(SettingsPath, path.Substring(11));
            
            if (path.StartsWith("data://"))
                return Path.Combine(DataPath, path.Substring(7));
