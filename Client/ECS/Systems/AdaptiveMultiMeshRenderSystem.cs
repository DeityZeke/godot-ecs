...
namespace Client.ECS.Systems
{
    ...
    public sealed class AdaptiveMultiMeshRenderSystem : BaseSystem
    {
        ...
        public override void OnInitialize(World world)
        {
            ...
            GD.Print($"[AdaptiveMultiMeshRenderSystem] Initialized. Target: {StartingCapacity:N0} Max: {MaxCapacity:N0}");
        ...
        private void UpdateFPSMonitoring(double delta)
        {
            ...
            if (!warmupComplete)
            {
                ...
                {
                    warmupComplete = true;
                    GD.Print($"[AdaptiveMultiMeshRenderSystem] Warmup complete, FPS monitoring active");
                }
                return;
            }
            ...
            if (averageFPS < MinimumFPS && !performanceLimited)
            {
                ...
                GD.Print($"[AdaptiveMultiMeshRenderSystem] Performance limit at {averageFPS:F1} FPS");
        ...
        private void HandleBatchAddition(double delta)
        {
            ...
            else if (currentVisualCapacity < targetVisualCapacity)
            {
                ...
                GD.Print($"[AdaptiveMultiMeshRenderSystem] Starting batch addition: " +
                        $"{currentVisualCapacity:N0} || {targetVisualCapacity:N0}");
            }
        }
        ...
        private void AddVisualBatch()
        {
                ...
                currentVisualCapacity >= MaxCapacity)
            {
                ...
                GD.Print($"[AdaptiveMultiMeshRenderSystem] Target reached: {currentVisualCapacity:N0}");
                return;
            }
            ...
            if (maxInstances > 0 && warmupComplete)
            {
                ...
                if (targetUpdates > _currentUpdatesPerFrame)
                {
                    ...
                    if (_framesAtCurrentTier >= TIER_CHANGE_DELAY && averageFPS >= MinimumFPS + 5f)
                    {
                        ...
                        GD.Print($"[AdaptiveMultiMeshRenderSystem] Increased to {_currentUpdatesPerFrame:N0} updates/frame (FPS: {averageFPS:F1})");
                    }
                }
                ...
                else if (averageFPS < MinimumFPS + 2f && _currentUpdatesPerFrame > BaseUpdatesPerFrame)
                {
                    ...
                    _framesAtCurrentTier = 0;
                    GD.Print($"[AdaptiveMultiMeshRenderSystem] Decreased to {_currentUpdatesPerFrame:N0} updates/frame (FPS: {averageFPS:F1})");
                }
